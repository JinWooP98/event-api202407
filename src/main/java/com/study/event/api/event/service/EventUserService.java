...
public class EventUserService {
    ...
    private void generateAndSendCode(String email, EventUser eventUser) {
        ...
        EmailVerification verification = EmailVerification.builder()
                ...
                .eventUser(eventUser) // Fk, 엔터티 자체를 주면 알아서 아이디를 받아와 사용(JPA 에서는)
                .build();
    ...
    public String sendVerificationEmail(String email) {
        ...
        try {
            ...
            // 이메일 제목 설정
            mimeMessageHelper.setSubject("[인증메일] 중앙정보스터디 가입 인증 메일입니다.");
            // 이메일 내용 설정
    ...
    public boolean isMatchCode(String email, String code) {
        ...
        if(eventUser != null) {
            ...
            if(ev != null && ev.getExpiryDate().isAfter(LocalDateTime.now()) && ev.getVerificationCode().equals(code)) {
    ...
    public boolean notFinish(String email) {
        ...
        if(!eventUser.isEmailVerified() || eventUser.getPassword() == null) {
            // 기존 인증코드가 있는 경우 삭제
            EmailVerification ev = emailVerificationRepository.findByEventUser(eventUser).orElseThrow(null);
    ...
    public LoginResponseDto authenticate(final LoginRequestDto dto) {
        ...
        // 로그인 성공
        // 인증정보를 어떻게 관리할 것인가? 세연 or 쿠키 or 토큰
